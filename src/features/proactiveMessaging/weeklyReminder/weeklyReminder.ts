...
export class WeeklyReminder {
    ...
    async add(notification: INotification | INotification[]) {
        ...
        if (notifications && notifications.length) {
            ...
            this.logger.logInfo(
                `Add new weekly notification ${notification.date} with message: ${notification.message}`
            );
        ...
        for (let index = 0; index < references.length; index++) {
            ...
            try {
                ...
                this.logger.logError(
                    `Scheduling weekly Reminder for conversation {${reference.conversation.id}} failed!
            ...
            const scheduledNotification = cron.schedule(dateExpression, () => {
                sendEventCallback(reference, async (turnContext: TurnContext) => {
                    try {
                        ...
                        await turnContext.sendActivity(notification.message.trim() ? notification.message : message);
                    } catch (error) {
